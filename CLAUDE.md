...
- **Staleness rule:** `modifiedTime > 7 days ago` AND `dueDate is None` AND `startDate is None` AND `status == 0` (Open) AND task `kind != "NOTE"` AND task is in a non-shared, non-archived, non-Note list (project `kind == "TASK"` or missing) AND task is not tagged `someday` (manual opt-out — see `SKIP_TAG`). Not `createdTime` — `createdTime` is immutable and would tag long-running active work.
